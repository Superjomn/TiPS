...
void NaiveBuffer::Require(size_t size) {
...
void NaiveBuffer::Consume(size_t bytes) {
  CHECK_LE(bytes, free_size()) << "No free memory of " << bytes << ", should Require the memory first";
  cursor_ += bytes;
  // Consume is used in both readonly and mutable mode to move the write/read
  // cursor, so we don't check mutable mode here.
}

void NaiveBuffer::SaveToFile(const std::string &filename) const {
...
void NaiveBuffer::AppendToFile(const std::string &filename) const {
...
void NaiveBuffer::LoadFromFile(const std::string &filename, const size_t &offset, const size_t &size) {
  ...
  if (fread(reinterpret_cast<char *>(&bytes_[0]), 1, buffer_size, fp) != buffer_size) {
    fclose(fp);
...
void NaiveBuffer::LoadFromMemory(void *buffer, size_t buffer_size) {
  Require(buffer_size);
...
void StringBuilder::Flush() {
  ...
  memcpy(table()->cursor(), reinterpret_cast<const byte_t *>(data_.c_str()), str_len);
...
#define NEW_PRIMARY_BUILDER_IMPL(T, name__)                                       \
  PrimaryBuilder<T> *StructBuilder::New##name__(const std::string &name, T val) { \
...
StringBuilder *StructBuilder::NewStr(const std::string &name, const std::string &val) {
  using type = StringBuilder;
  field_builders_.Set(name, std::unique_ptr<type>(new type(table(), val)));
  return static_cast<type *>(field_builders_.Get(name).get());
}

void StructBuilder::Flush() {
