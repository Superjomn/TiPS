...
from tips.tensorflow.ops import rank
...
def create_distributed_optimizer(keras,
    ...
    class _DistributedOptimizer(keras.optimizers.Optimizer):
        ...
        def _allreduce(self, grads, vars):
            ...
            if self._agg_helper:
                return self._agg_helper.compute_gradients(
                    tuple(grads), tuple(vars))
            else:
...
if hasattr(tips, 'broadcast_global_variables'):

    def broadcast_global_variables(backend, root_rank):
        return _eval(backend, tips.broadcast_global_variables(root_rank))
...
def allgather(backend, value, name):
    return _eval(backend, tips.allgather_op(tf.constant(value, name=name)))


def broadcast(backend, value, root_rank, name):
    return _eval(backend,
                 tips.broadcast_op(tf.constant(value, name=name), root_rank))
...
def load_model(keras, wrap_optimizer, optimizer_modules, filepath,
    ...
    if custom_objects is not None:
        tips_objects.update(custom_objects)

    return keras.models.load_model(filepath, custom_objects=tips_objects)
