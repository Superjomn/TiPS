    ...
    def compute_gradients(self, grads):
        ...
        for idx, grad in enumerate(grads):
            ...
            if self.sparse_as_dense and isinstance(grad, tf.IndexedSlices):
                ...
                raise ValueError("IndexedSlices are not supported when "
        ...
        resulting_grads = tf.cond(
            ...
            true_fn=lambda: _all_reduce_and_clear_aggregated_variables(
                resulting_grads),
    ...
    def apply_gradients(self, apply_grads_closure, optimizer, *args, **kwargs):
        ...
        def is_aggregation_step():
            if _POST_TF_2_4_0:
                ...
                return tf.equal(
                    tf.add(self.counter, 1), self.backward_passes_per_step)
